...
if __name__ == "__main__":
    ...
    judge_model_name = "qwen3-235b-a22b-2507"
    ...
    logger.info(f"Average Score: {float(np.mean([item['original_score'] for item in dataset if item['original_score'] != -1 and item['original_score'] != 9]))}")
    ...
    # # show average scores of each question category excluding perfect samples
    # categories = set([item['category'] for item in dataset])
    # logger.info("-----------------------")
    # logger.info("Average scores of each question category excluding perfect samples:")
    # for category in categories:
    #     category_samples = [item for item in dataset if item['category'] == category]
    #     logger.info(f"{category}:\t{float(np.mean([item['original_score'] for item in category_samples if item['original_score'] != -1 and item['original_score'] != 9])):.2f}")
    #     logger.info(f"Number of samples: {len(category_samples)}")
    #     logger.info(f"Number of avaiable samples: {sum([1 for item in category_samples if item['original_score'] != -1 and item['original_score'] != 9])}")
    #     logger.info(f"Number of perfect samples: {sum(1 for item in category_samples if item['original_score'] >= 9)}")
    #     logger.info(f"--------------------------------")
