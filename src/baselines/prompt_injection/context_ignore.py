...
from src.baselines.prompt_injection.base import IgnoreAttack
...
async def main():
    parser = argparse.ArgumentParser(description="Context ignore attack for LLM-as-a-Judge")
    ...
    setup_logging(task_name="context_ignore_attack")
    ...
    for traj_idx, traj in enumerate(ucb_trajectories):
        ...
        # Generate context ignore attack prompts
        injected_prompt = IGNORE_INJECTED_PROMPT_SET[traj.metadata.judge_type]
        new_answers = [IgnoreAttack(injected_prompt).generate_attack_prompt(original_answer) for original_answer in original_answers]
        
        # Load judge model and evaluate context ignore attack answers
        judge_type = get_judge_type(traj.metadata.judge_type)
        judge_model_name = get_implementation_name(traj.metadata.judge_backbone)
        logger.info(f"  Evaluating {len(new_answers)} context ignore attack answers...")
        ...
        traj_meta_info = {
            "strategy": "context_ignore_attack",
        ...
        save_trajectories(traj_to_save, args.save_trajectory_path, f"context_ignore_attack")
     
    logger.info(f"Context ignore attack completed!")
