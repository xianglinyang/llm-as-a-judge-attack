...
from src.baselines.prompt_injection.base import EscapeAttack
...
async def main():
    parser = argparse.ArgumentParser(description="Escape character prompt injection attack for LLM-as-a-Judge")
    ...
    setup_logging(task_name="escape_attack")
    ...
    for traj_idx, traj in enumerate(ucb_trajectories):
        ...
        # Generate escape attack prompts
        injected_prompt = ESCAPE_INJECTED_PROMPT_SET[traj.metadata.judge_type]
        new_answers = [EscapeAttack(injected_prompt).generate_attack_prompt(original_answer) for original_answer in original_answers]
        
        # Load judge model and evaluate escape attack answers
        judge_type = get_judge_type(traj.metadata.judge_type)
        judge_model_name = get_implementation_name(traj.metadata.judge_backbone)
        logger.info(f"  Evaluating {len(new_answers)} escape attack answers...")
        ...
        if judge_type in [JudgeType.POINTWISE, JudgeType.MT_BENCH, JudgeType.MLR_BENCH]:
            ...
            escape_attack_scores, _, escape_attack_explanations = await reward_calculator.calculate_batch_reward(questions, new_answers, original_scores)
            ...
            escape_attack_scores, _, escape_attack_explanations = await reward_calculator.calculate_batch_reward(questions, new_answers, original_scores, baseline_responses)
        else:
        ...
        for i in range(len(traj.trajectories)):
            ...
            if original_score >= escape_attack_scores[i]:
                result_item = {
                ...
                result_item = {
                    ...
                    "final_score": float(escape_attack_scores[i]),
                    "final_explanation": escape_attack_explanations[i],
        ...
        save_trajectories(traj_to_save, args.save_trajectory_path, f"escape_attack")
    
    logger.info(f"Escape attack completed!")
