...
class ContextualLinEpsilonGreedyAgent(ContextualLinBanditAgent):
    def __init__(self, n_features: int, llm_agent: ModelWrapper, embedding_model: TextEncoder, judge_type: JudgeType, judge_model_backbone: str, reward_type: str = "relative", epsilon: float = 0.1, lambda_reg: float = 1.0, answer_position: str = "first"):
        """
        ...
        Args:
            ...
            epsilon (float): Exploration parameter. Controls the probability of random exploration.
    ...
    def predict(self, context_x):
        """
        Predicts the expected reward for each arm given the context.

        Args:
            context_x (np.array): A (n_features x 1) column vector representing the context.

        Returns:
            np.array: A (n_arms x 1) vector of expected rewards for each arm.
        """
    ...
    def batch_predict(self, context_x_list):
        ...
        Returns:
            np.array: A (n_samples, n_arms, 1) tensor of expected rewards for each arm.
        """
    ...
    def choose_arm(self, context_x):
        ...
        if prob < self.epsilon:
            chosen_arm_idx = np.random.choice(range(self.n_arms))
        else:
    ...
    def batch_choose_arm(self, context_x_list):
        ...
        for i in range(n_samples):
            if probs[i] < self.epsilon:
                chosen_arm_idxs[i] = np.random.choice(range(self.n_arms))
            else:
...
async def main(args):
    ...
    agent = ContextualLinEpsilonGreedyAgent(args.n_features, llm_agent, embedding_model, judge_type, judge_model_backbone, args.reward_type, args.epsilon, args.lambda_reg, args.answer_position)
    ...
    save_trajectories(trajectories_to_save, args.save_trajectory_path, f"epsilon_greedy")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--judge_type", type=str, default="pointwise", choices=["pointwise", "pairwise", "pairwise_fine_grained", "alpaca_eval", "arena_hard_auto", "mt_bench"])
