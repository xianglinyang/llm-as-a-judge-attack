...
class ContextualLinEpsilonGreedyAgent(ContextualLinBanditAgent):
    def __init__(self, n_features: int, llm_agent: ModelWrapper, embedding_model: TextEncoder, judge_type: JudgeType, judge_model_backbone: str, reward_type: str = "relative", epsilon: float = 0.1, lambda_reg: float = 1.0):
        """
...
async def main(args):
    ...
    agent = ContextualLinEpsilonGreedyAgent(args.n_features, llm_agent, embedding_model, judge_type, judge_model_backbone, args.reward_type, args.epsilon, args.lambda_reg)
