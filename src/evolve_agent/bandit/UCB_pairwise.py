...
from src.llm_evaluator import load_judge_model, JudgeModel
...
class ContextualLinUCBAgent(ContextualLinBanditAgent):
    def __init__(self, n_features: int, llm_agent: ModelWrapper, embedding_model: TextEncoder, llm_evaluator: JudgeModel, reward_type: str = "relative", alpha: float = 1.0, lambda_reg: float = 1.0, judge_type: str = "pairwise"):
        """
...
if __name__ == "__main__":
    ...
    parser.add_argument("--judge_type", type=str, default="alpaca_eval", choices=["general", "alpaca_eval", "arena_hard_auto"])
    ...
    llm_evaluator = load_judge_model(args.judge_type, args.judge_model_name)

    dataset = load_dataset_for_exploration(args.data_dir, args.dataset_name, args.response_model_name, args.judge_model_name)
    baseline_dataset = load_dataset_for_exploration(args.data_dir, args.dataset_name, args.baseline_response_model_name, args.judge_model_name)
    ...
    pairwise_scores, pairwise_explanations = llm_evaluator.batch_pairwise_score(question_list, init_response_list, baseline_response_list)
    ...
    for idx in tqdm(range(len(dataset))):
        ...
        baseline_response = baseline_dataset[idx]['output']
        pairwise_score = pairwise_scores[idx]
        pairwise_explanation = pairwise_explanations[idx]

        if pairwise_score is None:
            continue
        elif pairwise_score > 0:
            test_results.append({
                "category": category,
                ...
                "pairwise_explanation": pairwise_explanation,
    ...
    baseline_dataset_for_exploration = [baseline_dataset[idx] for idx in selected_idxs]
    ...
    baseline_response_list = [item['output'] for item in baseline_dataset_for_exploration]
    category_list = [item['category'] for item in dataset_for_exploration]

    logger.info(f"Initializing the agent...")
    agent = ContextualLinUCBAgent(args.n_features, llm_agent, embedding_model, llm_evaluator, args.reward_type, args.alpha, args.lambda_reg, judge_type="pairwise")
    ...
    if args.test_mode == "online":
        ...
        trajectories = agent.online_learning(question_list, init_response_list, original_score_list, original_explanation_list, args.pool_size, args.Budget, init_policy=True, baseline_response_list=baseline_response_list)
        logger.info(f"Online learning finished.")
        logger.info("-"*100)
    elif args.test_mode == "single":
        logger.info(f"Running single exploration...")
        trajectories = agent.batch_explore(question_list, init_response_list, original_score_list, original_explanation_list, args.pool_size, args.Budget, cold_start=True, baseline_response_list=baseline_response_list)
        logger.info(f"Single exploration finished.")
        logger.info("-"*100)
    elif args.test_mode == "random":
        logger.info(f"Running random exploration...")
        trajectories = agent.batch_explore_with_random_arm(question_list, init_response_list, original_score_list, original_explanation_list, args.pool_size, args.Budget, baseline_response_list=baseline_response_list)
    ...
    for question, response, category, original_score, original_explanation, pairwise_score, pairwise_explanation, trajectory in zip(question_list, init_response_list, category_list, original_score_list, original_explanation_list, pairwise_scores, pairwise_explanations, trajectories):
    ...
    for category in CATEGORIES:
        ...
        up_num = len([result for result in category_results if result["pairwise_score"] < result["final_score"] and not result["skip"]])
        down_num = len([result for result in category_results if result["pairwise_score"] > result["final_score"] and not result["skip"]])
        tie_num = len([result for result in category_results if result["pairwise_score"] == result["final_score"] and not result["skip"]])
        ...
        avg_improvement = np.mean([result["final_score"] - result["pairwise_score"] for result in category_results if not result["skip"]])
        avg_score_before = np.mean([result["pairwise_score"] for result in category_results if not result["skip"]])
    ...
    save_path = os.path.join(args.save_trajectory_path, f"ucb_pairwise_{args.test_mode}_{args.dataset_name}_{timestamp}.json")
    with open(save_path, "w") as f:
